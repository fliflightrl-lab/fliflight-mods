#!/usr/bin/env python3
"""v3: blur FIXÉ (overlay enable), zoom renforcé, texte réordonné, nouveaux noms."""
...
def build(clip, win_no, out_name, ass_name):
    ...
    fc = (
        ...
        "[sharp][blur]overlay=0:0:enable='between(t,20.4,24)'[base2];"
        f"[base2]subtitles={ass_name}[v];"
        "[1:a]volume=1.0,apad[m];"
        "[2:a]adelay=20400|20400[a_w];"
...
for clip in clips:
    ...
    if win_no == 7:
        continue
    build(clip, win_no, f"final_w{win_no:02d}.mp4", f"v3_{win_no:02d}.ass")
    print("built final_w%02d -> %s" % (win_no, os.path.basename(MUSIC_FOR[win_no])))
print("DONE")
